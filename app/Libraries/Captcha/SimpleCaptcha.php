...
class SimpleCaptcha
{
    ...
    public $wordsFile = __DIR__ . '/words/en.php';
    ...
    public function GenerateImage()
    {
        ...
        if(!$this->text) $this->text = $this->GetCaptchaText();
        ...
        /** Transformations */
        if (!empty($this->lineWidth)) $this->WriteLine();
    ...
    protected function GetRandomCaptchaText($length = null)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            if ($vocal) $text .= substr($vocals, mt_rand(0, 4), 1);
            else $text .= substr($words, mt_rand(0, 22), 1);
    ...
    function GetDictionaryCaptchaText($extended = false)
    {
        ...
        if (substr($this->wordsFile, 0, 1) == '/') $wordsfile = $this->wordsFile;
        else $wordsfile = $this->resourcesPath . '/' . $this->wordsFile;
        ...
        if ($extended) {
            ...
            foreach ($text as $i => $char) {
                if (mt_rand(0, 1) && in_array($char, $vocals)) $text[$i] = $vocals[mt_rand(0, 4)];
            }
